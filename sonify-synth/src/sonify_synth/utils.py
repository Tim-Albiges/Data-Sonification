...
data scaling/normalization, and WAV file export (Mono/Stereo).
"""
...
def save_wav(filename: str, sample_rate: int, data: np.ndarray) -> None:
    """
    Save a NumPy audio array to a standard 16-bit PCM WAV file.

    Supports both Mono (1D array) and Stereo (2D array of shape 2xN).

    Args:
        filename (str): The output file path (should end in .wav).
        sample_rate (int): The audio sample rate (e.g., 44100).
        data (np.ndarray): The audio signal array.
    """
    ...
    # Convert to 16-bit integer for standard WAV format compatibility
    audio_int16 = (data * 32767).astype(np.int16)

    # Scipy expects stereo data as (Samples, Channels).
    # If our data is (2, Samples), we must transpose it.
    if data.ndim == 2:
...
def scale_data(data: np.ndarray, out_min: float, out_max: float) -> np.ndarray:
    ...
    # Edge case: If all data points are identical, return the base value
    if d_max == d_min:
